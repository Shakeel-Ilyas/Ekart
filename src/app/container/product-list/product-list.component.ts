...
@Component({
  ...
  standalone: true,
  imports: [NgFor, NgIf, ProductComponent, FilterComponent],
...
export class ProductListComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    this.totalProductCount = this.products.filter((prod) => {
      return prod.name
        .toLowerCase()
        .includes(changes['searchText'].currentValue);
    }).length;

    this.totalProductInStock = this.products.filter((prod) => {
      return (
        prod.name.toLowerCase().includes(changes['searchText'].currentValue) &&
        prod.is_in_inventory === true
      );
    }).length;

    this.totalProductOutOfStock = this.products.filter((prod) => {
      return (
        prod.name.toLowerCase().includes(changes['searchText'].currentValue) &&
        prod.is_in_inventory === false
      );
    }).length;
  }
